...
#include <stdlib.h> //biblioteca para funções de alocação de memória, controle de processos, conversões e outras
...
// estrutura para manter contadores de registros
typedef struct
{
...
{
	int c;
	// Limpa o buffer de entrada até encontrar uma nova linha ou EOF
	while ((c = getchar()) != '\n' && c != EOF)
		;
}
...
int adicionarLivro()
{
	FILE *arquivoContadores = fopen(CONTADORES_ARQUIVO, "rb+"); // Abre o arquivo de contadores para leitura e escrita
...
int atualizarLivro()
{
	...
	exibirLivro(&livro);
	printf("\nDigite as novas informações (pressione Enter para manter o valor "
		   "atual):\n");
	...
	if (strlen(entrada) > 1)
	{
		...
		{
			printf("Erro: Não é possível reduzir as cópias abaixo da quantidade "
...
// Função para excluir um livro do catálogo
int excluirLivro()
{
	char isbn[TAM_ISBN];
	printf("\n--- Excluir Livro ---\n");
...
// Função para exibir todos os livros do catálogo
void exibirTodosLivros()
{
	FILE *arquivoLivros = fopen(LIVROS_ARQUIVO, "rb");
	...
	printf("%-15s %-30s %-20s %-10s %s\n", "ISBN", "Título", "Autor",
		   "Disponíveis", "Localização");
	imprimirSeparador();

	Livro livro;
	for (int i = 0; i < contadores.contadorLivros; i++)
	{
		if (!fread(&livro, sizeof(Livro), 1, arquivoLivros))
			break;
		if (livro.estaAtivo)
		{
			printf("%-15s %-30s %-20s %d/%-9d %s\n", livro.isbn, livro.titulo,
				   livro.autor, livro.copiasDisponiveis, livro.totalCopias,
				   livro.localizacao);
		}
	}
...
void pesquisarLivros()
{
	...
	case 1:
		...
		while (fread(&livro, sizeof(Livro), 1, arquivoLivros))
		{
			if (livro.estaAtivo)
			{
				char copia[TAM_TITULO];
				strcpy(copia, livro.titulo);
				paraMinusculas(copia);
				if (strstr(copia, termoBusca) != NULL)
				{
					exibirLivro(&livro);
					encontrados = 1;
				}
	...
	case 2:
		...
		while (fread(&livro, sizeof(Livro), 1, arquivoLivros))
		{
			if (livro.estaAtivo)
			{
				char copia[TAM_AUTOR];
				strcpy(copia, livro.autor);
				paraMinusculas(copia);
				if (strstr(copia, termoBusca) != NULL)
				{
					exibirLivro(&livro);
	...
	case 4:
		...
		while (fread(&livro, sizeof(Livro), 1, arquivoLivros))
		{
			if (livro.estaAtivo)
			{
				char copia[TAM_CATEGORIA];
				strcpy(copia, livro.categoria);
				paraMinusculas(copia);
				if (strstr(copia, termoBusca) != NULL)
				{
					exibirLivro(&livro);
...
int registrarUsuario(const Papel papel)
{
	...
	memset(novoUsuario.historicoEmprestimos, 0,
		   sizeof(novoUsuario.historicoEmprestimos));
	...
	if (arquivoContadores == NULL)
		arquivoContadores = fopen(CONTADORES_ARQUIVO, "wb");
	if (arquivoContadores != NULL)
	{
...
int emprestarLivro()
{
	...
	if ((novoEmprestimo.dataVencimento.ano % 4 == 0 && novoEmprestimo.dataVencimento.ano % 100 != 0) || (novoEmprestimo.dataVencimento.ano % 400 == 0))
		diasNoMes[1] = 29;
	...
	printf("Data de Vencimento: %02d/%02d/%d\n",
		   novoEmprestimo.dataVencimento.dia, novoEmprestimo.dataVencimento.mes,
		   novoEmprestimo.dataVencimento.ano);
	...
	if (arquivoReservas != NULL)
	{
		...
		if (fc)
		{
			fread(&contRes, sizeof(Contadores), 1, fc);
			fclose(fc);
		}
		else
			contRes.contadorReservas = 0;
		...
		for (int i = 0; i < contRes.contadorReservas; i++)
		{
			...
			if (!fread(&reserva, sizeof(Reserva), 1, arquivoReservas))
				break;
			if (reserva.estaAtivo && !reserva.foiAtendida && strcmp(reserva.idUsuario, idUsuario) == 0 && strcmp(reserva.isbn, isbn) == 0)
			{
...
int devolverLivro()
{
	...
	while (1)
	{
		...
		if (!fread(&emprestimo, sizeof(Emprestimo), 1, arquivoEmprestimos))
			break;
		if (emprestimo.estaAtivo && !emprestimo.foiDevolvido && strcmp(emprestimo.idUsuario, idUsuario) == 0 && strcmp(emprestimo.isbn, isbn) == 0)
		{
			emprestimoExiste = 1;
			break;
		}
	...
	if (!emprestimoExiste)
	{
		printf("Erro: Nenhum empréstimo ativo encontrado para este usuário e "
			   "livro.\n");
		fclose(arquivoEmprestimos);
		return 0;
	}
...
int renovarLivro()
{
	...
	{
		printf("Erro: Não é possível renovar um livro atrasado. Devolva-o "
			   "primeiro.\n");
		fclose(arquivoEmprestimos);
		return 0;
	}
	...
	int diasNoMes[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	if ((emprestimo.dataVencimento.ano % 4 == 0 && emprestimo.dataVencimento.ano % 100 != 0) || (emprestimo.dataVencimento.ano % 400 == 0))
		diasNoMes[1] = 29;
	...
	printf("Livro renovado com sucesso!\nNova Data de Vencimento: "
		   ...
		   emprestimo.dataVencimento.ano, emprestimo.renovacoesUtilizadas,
		   MAX_RENOVACOES);
	return 1;
}
		...
		if (emprestimo.estaAtivo && !emprestimo.foiDevolvido)
		{
			...
			if (encontrarLivroPorISBN(emprestimo.isbn, &livro) != -1)
			{
				printf(
					...
					emprestimo.dataEmprestimo.dia, emprestimo.dataEmprestimo.mes,
					emprestimo.dataEmprestimo.ano, emprestimo.dataVencimento.dia,
	...
	while (fread(&emprestimo, sizeof(Emprestimo), 1, arquivoEmprestimos))
	{
		if (emprestimo.estaAtivo && !emprestimo.foiDevolvido && strcmp(emprestimo.idUsuario, idUsuario) == 0)
		{
			...
			if (encontrarLivroPorISBN(emprestimo.isbn, &livro) != -1)
			{
				...
				if (diasRestantes < 0)
					printf(
						"Status: ATRASADO por %d dias - Devolva imediatamente!\n",
						-diasRestantes);
				else
					printf("Status: %d dias restantes\n", diasRestantes);
				encontrados++;
...
int fazerReserva(const char *idUsuario)
{
	...
	if (livro.copiasDisponiveis > 0)
	{
		printf("Este livro está atualmente disponível. Não é necessário "
			   "reservar.\n");
		return 0;
	}

	// "ab+" para criar o arquivo se não existir
	...
	while (fread(&reserva, sizeof(Reserva), 1, arquivoReservas))
	{
		if (reserva.estaAtivo && strcmp(reserva.idUsuario, idUsuario) == 0)
		{
			...
			if (encontrarLivroPorISBN(reserva.isbn, &livro) != -1)
			{
				printf("\nID da Reserva: %d\nLivro: %s\nData: "
					   "%02d/%02d/%d\nStatus: %s\n",
					   reserva.idReserva, livro.titulo, reserva.dataReserva.dia,
			...
			if (diasAtraso > 0)
			{
				...
				if (encontrarLivroPorISBN(emprestimo.isbn, &livro) != -1)
				{
					printf("%-30s %-20s %02d/%02d/%d %13d\n", livro.titulo,
						   emprestimo.idUsuario, emprestimo.dataVencimento.dia,
						   emprestimo.dataVencimento.mes,
...
void gerarRelatorioMultas()
{
	...
	if (!arquivoUsuarios)
	{
		printf("Erro ao abrir usuários!\n");
		return;
	}
	FILE *arquivoEmprestimos = fopen(EMPRESTIMOS_ARQUIVO, "rb");
	if (!arquivoEmprestimos)
	{
		printf("Erro ao abrir empréstimos!\n");
		fclose(arquivoUsuarios);
		return;
	}
		...
		if (usuario.estaAtivo)
		{
			...
			if (multaUsuario > 0)
			{
				printf("%-20s %-30s R$%-14.2f R$%.2f\n", usuario.id,
					   usuario.nome, multaUsuario, usuario.multasTotais);
...
void gerarEstatisticasUso()
{
	...
	int emprestimosAtivos = 0, emprestimosDevolvidos = 0,
	...
	printf("Total de Empréstimos: %d\nEmpréstimos Ativos: %d\nEmpréstimos "
		   ...
		   contadores.contadorEmprestimos, emprestimosAtivos,
		   emprestimosDevolvidos, emprestimosAtrasados,
		   contadores.contadorEmprestimos > 0
			   ? ((float)emprestimosDevolvidos / contadores.contadorEmprestimos * 100)
			   : 0);
}
			...
			do
			{
				...
				case 1:
				{
					...
					case 3:
						...
						break;
					default:
						printf("Papel inválido. Usando LEITOR.\n");
						break;
					}
	...
	while (fread(&usuario, sizeof(Usuario), 1, arquivoUsuarios))
	{
		if (usuario.estaAtivo)
		{
			printf("%-10s %-20s %-15s %d\n", usuario.id, usuario.nome,
				   papelParaTexto(usuario.papel), usuario.contadorEmprestimos);
		}
	}
	fclose(arquivoUsuarios);
}
...
int main()
{
	...
	while (1)
	{
		...
		{
			printf(
				"Nenhum usuário encontrado! Criando administrador padrão...\n");
